...
vi.mock('../src/services/player.js', () => ({
  default: class {
    ...
    constructor(fileCache: unknown, config: unknown, guildId: string, findAudioFallback: (song: unknown) => Promise<unknown>) {
...
describe('per-guild Player isolation', () => {
  it('reuses one Player within a guild and creates a separate Player for another guild', async () => {
    ...
    const manager = new PlayerManager(fileCache as never, {} as never, youtubeAPI as never);
